...
pub struct Shift {
    ...
    ///
    /// The location should be based on where the employee clocked in.
    pub location_id: Option<String>,
    /// The read-only convenience value that is calculated from the location based on the location_id.
    ///
    /// Format: the IANA timezone database identifier for the location timezone.
    pub timezone: Option<String>,
    /// RFC 3339; shifted to the location timezone + offset.
    ///
    /// Precision up to the minute is respected; seconds are truncated.
    ///
    /// Min Length 1
    pub start_at: String,
    /// RFC 3339; shifted to the timezone + offset.
    ///
    /// Precision up to the minute is respected; seconds are truncated.
    pub end_at: Option<String>,
    /// Job and pay related information.
    ///
    /// If the wage is not set on create, it defaults to a wage of zero.
    ///
    /// If the title is not set on create, it defaults to the name of the role the employee is assigned to, if any.
    ...
    /// Used for resolving concurrency issues.
    ///
    /// The request fails if the version provided does not match the server version at the time of the request.
    ///
