...
pub struct CustomerCustomAttributeFilterValueV20230925 {
    /// A filter for a query based on the value of an Email-type custom attribute. This filter is case-insensitive and can include exact or fuzzy, but not both.
    ///
    /// For an exact match, provide the complete email address.
    ///
    /// For a fuzzy match, provide a query expression containing one or more query tokens to match against the email address. Square removes any punctuation (including periods (.), underscores (_), and the @ symbol) and tokenizes the email addresses on spaces. A match is found if a tokenized email address contains all the tokens in the search query, irrespective of the token order. For example, Steven gmail matches steven.jones@gmail.com and mygmail@stevenbakery.com.
    pub email: Option<CustomerTextFilterV20230925>,
    /// A filter for a query based on the value of a PhoneNumber-type custom attribute. This filter is case-insensitive and can include exact or fuzzy, but not both.
    ///
    ...
    pub phone: Option<CustomerTextFilterV20230925>,
    /// A filter for a query based on the value of a String-type custom attribute. This filter is case-insensitive and can include exact or fuzzy, but not both.
    ///
    ...
    pub text: Option<CustomerTextFilterV20230925>,
    ...
    pub selection: Option<FilterValueV20230925>,
    ...
    pub date: Option<TimeRangeV20230925>,
    /// A filter for a query based on the value of a Number-type custom attribute, which can be an integer or a decimal with up to 5 digits of precision.
    ///
    ...
    pub number: Option<FloatNumberRangeV20230925>,
    /// A filter for a query based on the value of a Boolean-type custom attribute.
    pub boolean: Option<bool>,
    /// A filter for a query based on the value of a Address-type custom attribute. The filter can include postal_code, country, or both.
    pub address: Option<CustomerAddressFilterV20230925>,
}
