...
pub async fn from_response_to_create_customer_response(
    response: Response,
    ...
    if response.status().is_success() {
        let parsed = match api_version {
            SquareApiVersion::V20230925 => {
                let parsed: Result<CreateCustomerResponseV20230925, Error> =
                    response.json::<CreateCustomerResponseV20230925>().await;
                parsed
            }
        };
